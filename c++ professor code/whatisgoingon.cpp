//	symbol.cpp -	Last revised 12/23/02
// (c) Copyright 2003 Robert M. Siegfried All Rights Reserved
#include "symbol.h"
...
void symboltable::setattrib(int tabindex, semantictype symbol, tokentype token)
{
    ...
    if (gettok_class(tabindex) == tokidentifier && thisproc.proc != -1)
...
int symboltable::labelscope(int procindex)
{
    ...
    numbytes -= 2;
    for (symptr = (getivalue(procindex) == 0) ? procindex + 1 : attribtable[oldsymptr].scopenext;
